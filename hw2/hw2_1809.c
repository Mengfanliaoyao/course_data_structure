...
#endif
    if(tasks[i].paths->count&&tasks[i].paths->paths[0]->edgeList->count)
...
#else
    tasks[task_id].tree = build_balance_binary_tree(tasks[task_id].paths->paths[0]);
#endif
#if DEBUG
...
#else
    int base_time = selectable_binary_tree(tasks[task_id].tree, memory_map);
    if(base_time >= 0)
    {
      select_task->pushBack(select_task, task_id);
      select_base_time->pushBack(select_base_time, base_time);
...
PathList* find_all_shortest_path(int start, int goal)
{
  ...
  while(!(pq->isEmpty(pq))) // dijasktra
  {
    ...
    for(int i = 0; i < nodes[u].links->count; i++)
    {
      ...
      {
        double new_dist = distance[u] + edge->distance;// - log(sucess_prob) - log(nodes[v].memory); // shorter path, through nodes with more memory better
...
BinaryTree* build_baseline_binary_tree(const Path* path)
{
  ...
  for(SizeT i = 0; i < edgelist->count - 1; i++)
  {
    tree_nodes[i + 1] = union_binary_tree(tree_nodes[i], tree_nodes[i+1]);
  }
  BinaryTree* tree = malloc(sizeof(BinaryTree));
  // verify ?
  tree->root = tree_nodes[edgelist->count-1];
  tree->height = edgelist->count;
  height_check_binary_tree(tree_nodes[edgelist->count-1], tree->height);
  free(tree_nodes);
  return tree;
}
BinaryTree* build_balance_binary_tree(const Path* path)
