...
sByteT task_cmp(const void* a, const void* b)
{
  ...
  if(count_a!=count_b) return (count_a>count_b)?1:-1;
  ...
  if(fabs(dist_a-dist_b)>1e-9) return (dist_a>dist_b)?1:-1;
...
int main(void)
{
  ...
  for(SizeT i = 0; i < task_queue->count; i++)
  {
    ...
    if(time_limit - tasks[task_id].paths->paths[0]->edgeList->count) // baseline tree height
    {
      tasks[task_id].tree = build_baseline_binary_tree(tasks[task_id].paths->paths[0]);
