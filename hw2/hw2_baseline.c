...
int shortest_path_len(int start, int goal)
{
  ...
  int* queue = malloc(node_count * sizeof(int)); // no repeat visit
  bool* visited = calloc(node_count, sizeof(bool)); // set initial value 0
  int head = 0, rear = 0;
  queue[rear++] = start;
  int min_len = -1, len = 0, len_ptr = rear;
  while(head < rear && min_len==-1) // stop when find the shortest path
  {
    int u = queue[head++];
    
    for(int i = 0; i < nodes[u].links->count; i++)
    {
      Link* edge = &links[nodes[u].links->data[i]];
...
PathList* find_all_shortest_path(int start, int goal)
{
  ...
  int min_len = shortest_path_len(start, goal);
  ...
  while(!(pq->isEmpty(pq))) // dijasktra
  {
...
#endif
    if(u == goal || path_len[u] > min_len) continue;
    ...
    for(int i = 0; i < nodes[u].links->count; i++)
    {
      ...
      {
        double new_dist = dist + edge->distance;// - log(sucess_prob) - 2 * log(nodes[v].memory); // shorter path, through nodes with more memory better
        ...
        if(new_dist < distance[v] || (fabs(new_dist - distance[v]) < 1e-9 && new_len < path_len[v])) // better dist or better path_len
        {
          ...
          for(SizeT i = 0; i < path_cache[u]->count; i++)
          {
            ...
            new_path->nodeList->pushBack(new_path->nodeList, v);
            new_path->edgeList->pushBack(new_path->edgeList, e);
  ...
  result->capacity = path_cache[goal]->count;
  result->paths = malloc(result->capacity * sizeof(Path*));
  for(SizeT i = 0; i < path_cache[goal]->count; i++)
  {
...
int selectable_binary_tree(BinaryTree* tree, int* time_map)
{
  static ByteT* occupancy_count = 0;
  if(!occupancy_count)
    occupancy_count = calloc(node_count, sizeof(ByteT));
  static ByteT* leaf_remain_count = 0;
  if(!leaf_remain_count)
    leaf_remain_count = calloc(node_count, sizeof(ByteT));
  
  // level order traversal
  BinaryTreeNode** queue = malloc((1<<tree->height) * sizeof(BinaryTreeNode*)); // for h-height binary tree, there are mostly 2^h nodes
