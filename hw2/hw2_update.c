...
int main(void)
{
  ...
  for(SizeT i = 0; i < task_queue->count; i++)
  {
    ...
    for(SizeT j = 0; j < tasks[task_id].paths->count; j++)
    {
      ...
      if(time_limit - path->nodeList->count >= 0) // time_limit > baseline tree height + 1
      {
        // search for baseline tree
        tasks[task_id].tree = build_baseline_binary_tree(path);
      ...
      // search for balance tree
      tasks[task_id].tree = build_balance_binary_tree(path);
      base_time = selectable_binary_tree(tasks[task_id].tree, memory_map);
      if(base_time >= 0)
      {
        select_task->pushBack(select_task, task_id);
...
PathList* find_all_shortest_path(int start, int goal) // dijasktra
{
  ...
  if(start == goal) return result; // empty
