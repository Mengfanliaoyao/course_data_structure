...
#if DEBUG
  printf("--------------\n");
  for(SizeT i = 0; i < user_queue_weight.size; i++)
  {
...
//  SizeT complexity = user_queue_weight.size * max_ris_for_user * entangle_generate_rate;
...
//  if(complexity<MAX_COMPLEXITY)
  {
    ...
    SizeT* dp_sheet = (SizeT*)malloc(sizeof(SizeT)*user_queue_weight.size*max_cost); 
    memset(dp_sheet, 0, sizeof(SizeT)*user_queue_weight.size*max_cost);
    ...
    for(SizeT i = 0; i < user_queue_weight.size; i++) // iterate all user
    {
      UD* u = &users[user_queue_weight.data[i]];
    ...
    SizeT best_index = 0;
    SizeT best_profit = 0;
    SizeT valid_user = user_queue_weight.size-1;
