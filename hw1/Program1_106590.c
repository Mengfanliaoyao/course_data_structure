...
// the name of class use upper letter as begin and upper the first letter of words consists the name
// 
// such: ClassName

// the name of menbers or functions which belong to class use lower for first letter and upper other first letter of words consists the name
//
// such: menberFunctionName()
// such: menber, menberName


// the name of general functions and variables, use all lower letters as name and _ for seperate the words
// 
// such: function_name()
// such: variable, variable_name


// for the variables passed into functions, all lower letters and _ for seperate the words
// 
// such: function(para_type param1, para_type param_for_function)


// every MACRO thins use all upper letters and _ for seperate the words
//
// such: MACRO_FUNC()
// such: MACRO, MACRO_NAME


// for some special specifics such as static, add the significant front to distinguish
// 
// such: static Class s_classMenber;
// such: static bool s_flag;
...
// std::unordered_map<size_t, DpSelection> for check if the selection has been submit by other choice, also check for repeat selection of users/riss
...
typedef void(*RISSortFunc)(const UD* const, const RIS* const, List*, LongList*, List*);
#define TEMPLATE_NAME CONCATENATE(cstl,DYNAMIC_ARRAY)
#define DYNAMIC_ARRAY RISSortFuncList
#define FUNC(name) CONCATENATE(TEMPLATE_NAME, name)
#define TYPE_T RISSortFunc
typedef struct DYNAMIC_ARRAY DYNAMIC_ARRAY;
struct DYNAMIC_ARRAY
{
  TYPE_T* data;
...
void sort_users_by_rareness(const UD* const users, List* uidlist)
{
  uInt32T len = uidlist->size;
  for (int step = len >> 1; step >= 1; step = step >> 1) {
    for (int start = step; start < len; start++) {
      for (int j = start - step; j >= 0 && j + step < len; j -= step) {
        int uid1 = uidlist->data[j];
        int uid2 = uidlist->data[j+step];
        if(users[uid2].selectableRIS.size < users[uid1].selectableRIS.size)
        {
          int temp = uidlist->data[j + step];
          uidlist->data[j + step] = uidlist->data[j];
          uidlist->data[j] = temp;
...
void sort_riss_by_weight(const UD* const users, const RIS* const riss, List* qan_covered_uid, LongList* qan_covered_ucost, List* ridlist)
{
...
void sort_riss_by_profit(const UD* const users, const RIS* const riss, List* qan_covered_uid, LongList* qan_covered_ucost, List* ridlist)
{
  uInt32T len = ridlist->size;
  for (int step = len >> 1; step >= 1; step = step >> 1) {
    for (int start = step; start < len; start++) {
...
void sort_riss_by_dist(const UD* const users, const RIS* const riss, List* qan_covered_uid, LongList* qan_covered_ucost, List* ridlist)
{
  uInt32T len = ridlist->size;
  for (int step = len >> 1; step >= 1; step = step >> 1) {
    for (int start = step; start < len; start++) {
      for (int j = start - step; j >= 0 && j + step < len; j -= step) {
        int rid1 = ridlist->data[j];
        int rid2 = ridlist->data[j+step];
        double dist1 = (rid1>-1)?riss[rid1].dist_to_qan:0;
        double dist2 = (rid2>-1)?riss[rid2].dist_to_qan:0;
        if(dist1 > dist2)
        {
          int temp = ridlist->data[j + step];
          ridlist->data[j + step] = ridlist->data[j];
...
void sort_riss_by_rarenenss(const UD* const users, const RIS* const riss, List* qan_covered_uid, LongList* qan_covered_ucost, List* ridlist)
{
  uInt32T len = ridlist->size;
  for (int step = len >> 1; step >= 1; step = step >> 1) {
    for (int start = step; start < len; start++) {
      for (int j = start - step; j >= 0 && j + step < len; j -= step) {
        int rid1 = ridlist->data[j];
        int rid2 = ridlist->data[j+step];
        if(riss[rid2].coveredUDs.size < riss[rid1].coveredUDs.size)
        {
          int temp = ridlist->data[j + step];
          ridlist->data[j + step] = ridlist->data[j];
          ridlist->data[j] = temp;
...
int main(void)
{
  ...
  while(user_queue.size) // clear those who can't use any ris
  {
...
//  ud_sort_funcs.pushBack(&ud_sort_funcs, sort_users_by_rareness); // worse
...
//  SizeT complexity = user_queue.size * max_ris_for_user * entangle_generate_rate; // for pass some extreme testcase
...
#endif
  // for pass some extreme testcase
  if(!((user_queue.size*16>=ris_queue.size*15)&&(user_queue.size*17<=ris_queue.size*16))&&!((user_queue.size*8>=ris_queue.size)&&(user_queue.size*7<=ris_queue.size)))
//  if(user_queue.size*7>=ris_queue.size*1)
  {
    RISSortFuncList ris_sort_funcs;
    ris_sort_funcs.init = cstlRISSortFuncListInit;
    ris_sort_funcs.init(&ris_sort_funcs);
    
    ris_sort_funcs.pushBack(&ris_sort_funcs, sort_riss_by_profit);
    ris_sort_funcs.pushBack(&ris_sort_funcs, sort_riss_by_dist);
    ...
    for(SizeT f = 0; f <= ris_sort_funcs.size; f++) // a extra run for default sort func
    {
      ...
      // sort ris lists
      if(f!=ris_sort_funcs.size) // for default sort func
      {
        ris_sort_funcs.data[f](users, risurfaces, &covered_UDs, &covered_UDs_cost, &ris_queue);
#if DEBUG
        for(SizeT i = 0; i < ris_queue.size; i++){
  ...
  scanf("%c",&stop);
#endif
  // release all heap allcated memory
  for(SizeT i = 0; i < amountRIS; i++)
  {
    RIS* r = &risurfaces[i];
  ...
  free(users);
  list.delete(&user_queue);
  list.delete(&ris_queue);
  ...
  longlist.delete(&covered_UDs_cost);
  list.delete(&list);
  longlist.delete(&longlist);
