...
TYPE_T FUNC(Pop)(DYNAMIC_ARRAY* this)
{
  if(this->size)
...
struct DpSelection
{
  BitsetT* bitsR; // record if the ris is selected
...
#define HEAP UDPriorQueueWeight
#define FUNC(name) CONCATENATE(TEMPLATE_NAME, name)
#define DATA_T UD
#define KEY_T Int32T
#define SIZE_T SizeT
...
struct HEAP
{
  ...
  SIZE_T capacity;
  SIZE_T limit;
  IndexMap keyToPosMap;
  void (*updatePriority)(HEAP* this, KEY_T key);
  void (*enqueue)(HEAP* this, KEY_T key);
  KEY_T (*dequeue)(HEAP* this);
  KEY_T (*popOut)(HEAP* this, SIZE_T index);
...
{
  IndexNode* idx = this->keyToPosMap.find(&this->keyToPosMap, this->indicies[index1]);
  idx->value = index2;
  idx = this->keyToPosMap.find(&this->keyToPosMap, this->indicies[index2]);
  idx->value = index1;
  
  KEY_T temp = this->indicies[index1];
  this->indicies[index1] = this->indicies[index2];
  this->indicies[index2] = temp;
}
void FUNC(HeapifyUp)(HEAP* this, SIZE_T index)
{
  while(index>1){
    ...
    DATA_T* t = this->data;
    KEY_T* idx = this->indicies;
    if(t[idx[parent]].weight<t[idx[index]].weight) // the weight of parent less then index
    {
...
void FUNC(HeapifyDown)(HEAP* this, SIZE_T index)
{
  while(1){
    ...
    DATA_T* t = this->data;
    KEY_T* idx = this->indicies;
    if(left<=this->size&&t[idx[left]].weight>t[idx[largest]].weight) // the weight of left larger than largest
      largest = left;
    if(right<=this->size&&t[idx[right]].weight>t[idx[largest]].weight) // the weight of right larger than largest
      largest = right;
    
    if(largest!=index)
    {
      FUNC(Swap)(this, index, largest);
      index = largest;
    }
    else
      break;
...
void FUNC(UpdatePriority)(HEAP* this, KEY_T key)
{
  IndexNode* idx = this->keyToPosMap.find(&this->keyToPosMap, key);
  if(!idx) return;
  
  SIZE_T index = idx->value;
  
  DATA_T* t = this->data;
  KEY_T* indicies = this->indicies;
  SIZE_T parent = (index>>1);
  if(parent>=1&&t[indicies[index]].weight>t[indicies[parent]].weight)
    FUNC(HeapifyUp)(this, index);
  else
  {
    SIZE_T left = (index<<1);
    SIZE_T right = left + 1;
    ...
    if(left<=this->size&&t[indicies[left]].weight>t[indicies[largest]].weight) // the weight of left larger than largest
      largest = left;
    if(right<=this->size&&t[indicies[right]].weight>t[indicies[largest]].weight) // the weight of right larger than largest
      largest = right;
    
    if(largest!=index)
      FUNC(HeapifyDown)(this, index);
  }
}
void FUNC(Enqueue)(HEAP* this, KEY_T key)
{
  IndexNode* idx = this->keyToPosMap.find(&this->keyToPosMap, key);
  if(idx)
  {
    FUNC(UpdatePriority)(this, key);
    return;
  }
  
  this->size++;
  if(this->size==this->capacity) // short of space
  {
    SIZE_T new_capacity = (this->capacity<<1);
    KEY_T* new_indicies = (KEY_T*)malloc(sizeof(KEY_T)*new_capacity);
    memcpy(new_indicies, this->indicies, sizeof(KEY_T)*this->capacity);
    free(this->indicies);
    this->indicies = new_indicies;
    new_indicies = NULL;
    this->capacity = new_capacity;
  }
  // insert node at the end
  this->indicies[this->size] = key;
  this->keyToPosMap.insert(&this->keyToPosMap, key, &this->size);
  // heapify up
  FUNC(HeapifyUp)(this, this->size);
  
}
KEY_T FUNC(Dequeue)(HEAP* this)
{
  if(!this->capacity) abort();
  // return top element
  KEY_T top = this->indicies[1];
  this->keyToPosMap.erase(&this->keyToPosMap, top);
  // move last element to top
  this->indicies[1] = this->indicies[this->size--]; // replace top by the end of heap and remove the end of heap
  IndexNode* idx = this->keyToPosMap.find(&this->keyToPosMap, this->indicies[1]);
  idx->value = 1;
  // heapify down
...
KEY_T FUNC(PopOut)(HEAP* this, SIZE_T index)
{
  if(index>this->size) return -2147483648;
  FUNC(Swap)(this, index, this->size); // swap with heap end
  KEY_T element = this->indicies[this->size--]; // return the selected element
  
  // now the origin heap end element is at index
  FUNC(UpdatePriority)(this, this->indicies[index]);
  
  return element;
}
bool FUNC(IsEmpty)(HEAP* this) {return (this->size==0);}
...
void FUNC(Init)(HEAP* this)
{
  this->size = 0;
  this->capacity = 2; // 0 and 1
  this->indicies = malloc(sizeof(KEY_T)*this->capacity);
  this->data = NULL; // need user bind before use
  this->limit = 0xFFFFFFFFFFFFFFFF; // defalut has no limit
  this->keyToPosMap.init = CstlIndexMapInit;
  this->keyToPosMap.init(&this->keyToPosMap);
  this->updatePriority = FUNC(UpdatePriority);
  this->enqueue = FUNC(Enqueue);
  this->dequeue = FUNC(Dequeue);
  this->popOut = FUNC(PopOut);
...
int main(void)
{
  ...
  UDPriorQueueWeight user_heap_weight;
  user_heap_weight.init = CstlUDPriorQueueWeightInit;
  user_heap_weight.init(&user_heap_weight);
  ...
  if(amountUD)
  {
    ...
    user_heap_weight.data = users; // bind actual users
  ...
  for(SizeT i = 0; i < covered_UD_amount; i++)
  {
    ...
    user_heap_weight.enqueue(&user_heap_weight, uid);
    u->cost = cost;
  ...
  if(amountRIS)
  {
    ...
    for(SizeT i = 0; i < amountRIS; i++)
    {
      ...
      for(SizeT j = 0; j < covered_amount; j++)
      {
        ...
        if(user_heap_weight.keyToPosMap.find(&user_heap_weight.keyToPosMap, uid))
          user_heap_weight.updatePriority(&user_heap_weight, uid);
        else
          user_heap_weight.enqueue(&user_heap_weight, uid);
      }
    }
...
//  ud_sort_funcs.pushBack(&ud_sort_funcs, sort_users_by_uid); // mean less
...
//  if(complexity<MAX_COMPLEXITY)
          ...
          for(SizeT j = 0; j < max_cost; j++){ // iterate for all the selection of last user
            ...
            if(dp_sheet[last_user_select]) // the last user selected
            {
              ...
              for(SizeT k = 0; k < u->selectableRIS.size; k++) // iterate for all the avalible ris for user
              {
                ...
                if(!(prev->data.bitsR[rid/64]&BIT(rid%64))) // the ris has not selected by current choose
                {
                  ...
                  if(select_cost<max_cost) 
                  {
                    if(dp_sheet[now_select]) // the choice is selected
                    {
                      ...
                      if(select_profit>dp_sheet[now_select]) // the choice is better, rewrite the now_select
                      {
                        ...
                        memcpy(now_select_dp_sheet->data.bitsR, prev->data.bitsR, bits_r_size);
                        now_select_dp_sheet->data.bitsR[rid/64] |= BIT(rid%64);
                    ...
                    else // the index of this user has not been selected
                    {
                      ...
                      s->bitsR = malloc(bits_r_size);
                      memcpy(s->bitsR, prev->data.bitsR, bits_r_size);
                      s->bitsR[rid/64] |= BIT(rid%64);
  ...
  {
    // double heap (weight sorted)
    SizeT bits_r_size = sizeof(BitsetT)*((amountRIS+63)/64);
    BitsetT* bits_r = (BitsetT*)malloc(bits_r_size);
    memset(bits_r, 0, bits_r_size);
    
    sort_users_by_cost(users, &user_queue); // use as cost heap
    SizeT cost_index = 0;
    
    IndexMap selection_map;
    selection_map.init = CstlIndexMapInit;
    selection_map.init(&selection_map);
    
    LongList empty_selection_list;
    longlist.init(&empty_selection_list);
    
    while(!user_heap_weight.isEmpty(&user_heap_weight)){
      // check if the top of heap selectable
      Int32T heap_top_uid = user_heap_weight.indicies[1];
      UD* u = &users[heap_top_uid];
      Int32T rid = u->selectableRIS.data[0];
      SizeT rcost = u->selectableRISCost.data[0];
      if(!(bits_r[rid/64]&BIT(rid%64))&&currentlist.cost+rcost<=entangle_generate_rate) // the top is selectable
      {
        user_heap_weight.dequeue(&user_heap_weight);
        // update the selection state
        bits_r[rid/64] |= BIT(rid%64);
        currentlist.cost+=rcost;
        currentlist.profit+=u->profit;
        if(!empty_selection_list.size) // the pushback list has no blank
        {
          list.pushBack(&currentlist.userList, u->id);
          list.pushBack(&currentlist.risList, rid);
        }
        else
        {
          SizeT insert_index = longlist.pop(&empty_selection_list);
          currentlist.userList.data[insert_index] = u->id;
          currentlist.risList.data[insert_index] = rid;
        }
        SizeT* node = malloc(sizeof(SizeT));
        *node = currentlist.userList.size;
        selection_map.insert(&selection_map, u->id, node);
        continue;
      }
      else // iterate whole heap
      {
        for(SizeT i = 2; i <= user_heap_weight.size;)
        {
          Int32T uid = user_heap_weight.indicies[i];
          UD* u = &users[uid];
          Int32T rid = u->selectableRIS.data[0];
          SizeT rcost = u->selectableRISCost.data[0];
          if(!(bits_r[rid/64]&BIT(rid%64))&&currentlist.cost+rcost<=entangle_generate_rate) // selectable
          {
            user_heap_weight.popOut(&user_heap_weight, i);
            // update the selection state
            bits_r[rid/64] |= BIT(rid%64);
            currentlist.cost+=rcost;
            currentlist.profit+=u->profit;
            ...
            *node = currentlist.userList.size;
            selection_map.insert(&selection_map, u->id, node);
            i--; // the origin element is replaced to heap[heap.size] and dropped, need to check for heap[i] again
          }
          else
            i++;
        }
        // update ans
        if(currentlist.profit>bestlist.profit)
        {
          while(empty_selection_list.size) // there are blanks in the current list need to deal with
          {
            SizeT insert_index = list.pop(&currentlist.userList); // for each blank in current list
            memmove(currentlist.userList.data+insert_index, currentlist.userList.data+(insert_index + 1), (currentlist.userList.size-insert_index-1)*sizeof(Int32T));
            currentlist.userList.size--;
            memmove(currentlist.risList.data+insert_index, currentlist.risList.data+(insert_index + 1), (currentlist.risList.size-insert_index-1)*sizeof(Int32T));
            currentlist.risList.size--;
          }
          bestlist.profit = currentlist.profit;
          bestlist.cost = currentlist.cost;
          list.copyFrom(&bestlist.userList, &currentlist.userList);
          list.copyFrom(&bestlist.risList, &currentlist.risList);
        }
        // must no source to add new selection
        while(1){
          SizeT remove_uid = user_queue.data[cost_index++];
          IndexNode* index_node = selection_map.find(&selection_map, remove_uid);
          if(index_node){ // the uid to move has selected
            // remove the select from current selection
            SizeT empty_index = index_node->value;
            longlist.pushBack(&empty_selection_list, empty_index);
            selection_map.erase(&selection_map, remove_uid);
            Int32T remove_rid = users[remove_uid].selectableRIS.data[0]; 
            bits_r[remove_rid/64] &= ~BIT(remove_rid%64);
            break;
          }
          else // from heap remove the uid
          {
            user_heap_weight.popOut(&user_heap_weight, user_heap_weight.keyToPosMap.find(&user_heap_weight.keyToPosMap, remove_uid)->value);
            // since we have iterated whole heap and no source to add, once we don't remove from selection there is no source to select too
            if(user_heap_weight.isEmpty(&user_heap_weight))
              break;
          }
        }
      }
